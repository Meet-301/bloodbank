...
if (isset($_POST['send'])) {
    ...
    // Fetch requirer's details 
    $requirer = "SELECT BloodUnits
               FROM tblblooddonars 
               WHERE id = :cid";
    $myquery = $dbh->prepare($requirer);
    $myquery->bindParam(':cid', $cid, PDO::PARAM_STR);
    $myquery->execute();
    $myresult = $myquery->fetch(PDO::FETCH_OBJ);

    if($myresult) {
        $bunits = (int) $myresult->BloodUnits;
    }

    if ($result) {
        $name = $result->FullName;
        $email = $result->EmailId;
        $contactno = $result->MobileNumber;
    }
    ...
    if ($requested_bunits > $max_allowed) {
        ...
        // Insert the actual requested units, not donor's total
